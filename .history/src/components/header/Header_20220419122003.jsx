...
import {isUserExists} from "../../services/api"
...
function Header() {
  const { user, isAuthenticated, logout, error, isLoading } = useAuth0();
...
useEffect(() => {
  (async () => {
    if (isAuthenticated && localStorage.getItem("autoriseUser") &&
    localStorage.getItem("autoriseUser") === user.n) {
